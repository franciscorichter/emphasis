...
tree_t simtree_pd(const std::vector<double>& pars,
  ...
  while (cbt < ct &&
    ...
    double next_event_time = cbt - std::log(u1) / rate_max;
...
// [[Rcpp::export(name = "simtree_dd_cpp")]]
tree_t simtree_dd(const std::vector<double>& pars,
                  double ct) {
 ...
 double cbt = 0.0;
  ...
  while (cbt < ct &&
    ...
    int N = N1 + N2;
    double lambda_ct = std::max(0.0, 
                                pars[1] + pars[2] * N);
    double rate_max = (lambda_ct + mu) * N;
    double u1 = std::uniform_real_distribution<>()(reng);
    double next_event_time = cbt - std::log(u1) / rate_max;
