...
$\mathrm{ED}$ is the per-lineage form of phylodiversity-dependence: it is defined on the complete tree, so hidden lineages count among a branch's descendants and contribute branches, and the augmentation supplies them. It needs the tree's topology (a `phylo`, not a branching-time vector). It has no *exact* proposal — a per-lineage covariate has no single survival probability — but the conditional (BDI) sampler serves it as a mean-field proposal: every lineage alive at $t$ is offered the rate the clade's average lineage has there, and the importance weights carry each lineage's own departure from it. The clade mean of fair-proportion ED is Faith's PD per lineage, which the sampler's backward–forward iteration already produces. Measured over ten trees a cell at $n \approx 100$ and 200 draws, it leaves 41 times the effective sample the thinning proposal does on `ned` trees at complete sampling and 1.7 times on `ed` trees, where the ED coefficient is the only thing bounding the clade and so is much the stronger effect. **The ranking against $\rho$ depends on the model, not on $\rho$ alone.** On `cr`, `dd` and `ned` the conditional proposal leads at every $\rho$ down to 0.2, its margin narrowing without crossing — 41× to 3.7× for `ned`. On `ed` it crosses between $\rho = 0.8$, where it still leads 1.87×, and $\rho = 0.6$, where it trails 2.6×. So: **the conditional proposal, except `ed` below $\rho \approx 0.8$.** (`sim/E6-proposal-rho.R` in the paper repository; `dev/mfc_ess.R` measured an earlier sampler, before births were attached in proportion to the rate, and its figures do not describe this one.) The gaussian link is not available with $\mathrm{ED}$. The same routine computes it in the simulator and in the likelihood (`inst/include/ed_covariate.hpp`), and `tests/testthat/test-ed.R` checks it against a fair-proportion computation written independently on `ape` trees. The C++ entry point `ed_fair_proportion(parent, birth, alive, t)` is internal and takes the packed representation rather than a `phylo`, so it is what the test drives and not a way for a caller to score a tree of their own. Its cost is quadratic in the *augmented* tree — every segment's tables over every alive lineage, built once per tree and cached across the parameter values an M-step tries — and the augmentation builds the complete clade the current parameters imply, so an $\mathrm{ED}$ fit is started near a plausible net rate: in practice from the estimate of the nested model (`ed` from `cr`, `ned` from `dd`) with the $\mathrm{ED}$ coefficients at zero. From a point at 1.7 times the generating net rate, a 54-tip tree at $\rho = 0.7$ augments to a median 6,586 nodes and the E-step's time runs out before 50 draws; from the `dd` estimate the same fit takes seconds.
