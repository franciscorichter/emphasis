...
$$
\lambda(s,t) = f\!\bigl(\beta_0,\; \beta_N N(t) + \beta_M M(t) + \beta_D D_s(t)\bigr), \quad
\mu(s,t) = f\!\bigl(\gamma_0,\; \gamma_N N(t) + \gamma_M M(t) + \gamma_D D_s(t)\bigr)
$$
...
The links are selected with `link = "linear"`, `"exponential"`, or `"gaussian"` (the quadratic-exponential form). The quadratic-exponential link produces a bell-shaped rate that peaks when the linear predictor $\eta = 1$, capturing intermediate optima such as a peak speciation rate at an optimal clade size.

The covariates form an **orthogonal basis**. Writing $I_s(t)$ for the isolation time of lineage $s$ (time since it last speciated), the raw phylogenetic diversity $P(t) = \sum_{s} I_s(t) = N(t)\,\bar I(t)$ is *not* used directly, because $P = N\bar I$ makes it collinear with richness. Instead $P$ is decomposed into a mean (global) and a deviation (local):

| Symbol     | Name                   | Definition                                                                |
| ---------- | ---------------------- | ------------------------------------------------------------------------- |
| $N(t)$   | Species richness       | Number of extant lineages at time $t$                                    |
| $M(t)$   | Mean pendant age       | $M(t) = P(t)/N(t) = \bar I(t)$, the clade's mean isolation time (maturity) |
| $D_s(t)$ | Focal deviation        | $D_s(t) = I_s(t) - M(t)$, the focal lineage's isolation relative to the mean; $\sum_s D_s = 0$ |

Because $\sum_s D_s(t) = 0$ at every instant, $D$ is orthogonal to the intercept and to the global covariates $N, M$ — the three axes measure distinct effects (diversity level, clade maturity, and per-lineage identity) rather than overlapping ones.
...
| `"ma"` | $M$      | $(\beta_0, \beta_M, \gamma_0, \gamma_M)$ | Regulation by clade maturity (mean age) |
| `"rd"` | $D_s$    | $(\beta_0, \beta_D, \gamma_0, \gamma_D)$ | Lineage-specific relative-isolation effects |

Models can also be given as a formula: `model = ~ N + M` activates both covariates (6 parameters). The shortcuts `"pd"` and `"ep"` are accepted as aliases for `"ma"` and `"rd"`.
