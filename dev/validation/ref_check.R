...
for (cond in 0:1) cat(sprintf("bd_loglik at ape MLE, cond=%d btorph=1: %.4f\n", cond,
  bd_loglik(pars1 = c(ape_mle[1], ape_mle[2], 0, 0), pars2 = c(0, cond, 1, 2, 0), brts = brts, missnumspec = 0)))
# fixed-parameter loglik surface: bd_loglik vs dd_loglik with huge K (should coincide)
ll_bd <- bd_loglik(pars1 = c(lam, mu, 0, 0), pars2 = c(0, 0, 1, 2, 0), brts = brts, missnumspec = 0)
ll_dd <- dd_loglik(pars1 = c(lam, mu, 1e6), pars2 = c(500, 1, 0, 1, 2, 0), brts = brts, missnumspec = 0)
