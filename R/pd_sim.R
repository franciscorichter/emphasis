...
sim_tree_pd_R <- function(pars, max_t) {
    ...
    while (t < max_t && N1 >= 1 && N2 >= 1) {
        ...
        N <- N1 + N2
        spec_rate <- max(0, pars[2] + pars[3] * N + ((P + N * (max_t - t) - t)/N) * pars[4])
        ...
        if (next_event_time < max_t) {
            focal_spec <- max(0, pars[2] + pars[3] * N + ((P + N * (next_event_time - t) - t)/N) * pars[4])
            ...
            if (stats::runif(1) < pt) {
                ...
                if (stats::runif(1) < focal_spec/(focal_spec + mu)) {
                  # speciation cat('speciation', '\n')
                  parent <- sample(which(tree[, 4] == -1), 1)

                  new_ID <- tree_ID
                  tree_ID <- tree_ID + 1
                  if (tree[parent, 3] < 0)
                    new_ID <- new_ID * -1  # 'b' clade
                  ...
                  # extinction pick random species cat('extinction', '\n')
    ...
    return(list(phy = phy, result = c(N, t, P)))
...
# time of extinction (or crown age), and 5) the break condition indicating why the simulation was stopped
# (options: no break (none), exceeded max_t, extinction, or exceeded max_lin).
#' @export
...
#' and the phylogenetic diversity at the time of extinction (or crown age).
#' @export
sim_tree_pd_grid <- function(mu_vec, lambda_vec, b_n_vec, b_p_vec, max_t, num_repl, max_N) {
    result <- explore_grid_cpp(mu_vec, lambda_vec, b_n_vec, b_p_vec, max_t, num_repl, max_N)
