...
emphasis <- function(brts, init_par, soc = 2, model, lower_bound = numeric(0), upper_bound = numeric(0), max_lambda = 500,
    xtol = 0.001, em_tol = 0.25, sample_size_tol = 0.005, verbose = FALSE, return_trees = FALSE, max_missing = 10000,
    burnin_sample_size = 200, pilot_sample_size = seq(100, 1000, by = 100), burnin_iterations = 20, num_threads = 0,
    conditional = NULL) {

    if (NULL != conditional) {
        stopifnot( inherits(conditional, "function")) 
    }
    ...
    mc <- mcEM_step(brts = brts, pars = init_par, sample_size = burnin_sample_size, model = model, soc = soc, max_missing = max_missing,
        max_lambda = max_lambda, lower_bound = lower_bound, upper_bound = upper_bound, xtol = xtol, num_threads = num_threads,
        return_trees = FALSE, verbose = FALSE, tol = em_tol, burnin = burnin_iterations, conditional)

    M <- mc$mcem
    pars <- c(mean(utils::tail(M$par1, n = nrow(M)/2)), mean(utils::tail(M$par2, n = nrow(M)/2)), mean(utils::tail(M$par3,
        n = nrow(M)/2)), mean(utils::tail(M$par4, n = nrow(M)/2)))
    ...
    for (s in pilot_sample_size) {
        ...
        mc <- mcEM_step(brts = brts, pars = pars, sample_size = s, model = model, soc = soc, max_missing = max_missing,
            max_lambda = max_lambda, lower_bound = lower_bound, upper_bound = upper_bound, xtol = xtol, num_threads = num_threads,
            return_trees = FALSE, verbose = FALSE, tol = em_tol, burnin = 10, conditional)
    ...
    while (n.r_old < n.r) {
        ...
        cat("\n", msg5, msg7, msg6, sep = "\n")
        mc <- mcEM_step(brts = brts, pars = pars, sample_size = sample_size, model = model, soc = soc, max_missing = max_missing,
            max_lambda = max_lambda, lower_bound = lower_bound, upper_bound = upper_bound, xtol = xtol, num_threads = num_threads,
            return_trees = FALSE, verbose = FALSE, tol = em_tol, burnin = 2, conditional)
...
mcEM_step <- function(brts, pars, sample_size, model, soc, max_missing, max_lambda, lower_bound, upper_bound, xtol,
    tol, burnin, num_threads, return_trees, verbose, conditional) {
    mcem <- NULL
    sde <- 10
    i <- 0
    times <- NULL
    while (sde > tol) {
        i <- i + 1
        results <- em_cpp(brts, pars, sample_size, maxN = 10 * sample_size, locate_plugin(model), soc, max_missing,
            max_lambda, lower_bound, upper_bound, xtol_rel = xtol, num_threads, return_trees, conditional)
        pars <- results$estimates
        mcem <- rbind(mcem, data.frame(par1 = pars[1], par2 = pars[2], par3 = pars[3], par4 = pars[4], fhat = results$fhat,
        ...
        if (i > burnin) {
            ...
            msg <- paste("Remaining time (burn-in): ", round(time_p_it * (burnin - i), digits = 0), "sec")
            cat("\r", msg)
        }
    }
