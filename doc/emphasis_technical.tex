...
over the search box $[\lb, \ub]$. The CEM at each iteration selects the
member of $g$ that minimises the KL divergence to the ``ideal'' importance
distribution $g^*$ that places all mass at the global maximiser of
$\fhat$:
\begin{equation}
  (\bm{\mu}^{(k+1)}, \bm{\Sigma}^{(k+1)}) = \argmin_{\bm{\mu},\bm{\Sigma}}
  \KL\!\left(g^* \;\big\|\; g(\cdot;\bm{\mu},\bm{\Sigma})\right).
\end{equation}
In the Gaussian case this CE update reduces to: set $\bm{\mu}^{(k+1)}$ to
the sample mean of the elite particles and $\bm{\Sigma}^{(k+1)}$ to their
sample covariance. Because the full covariance update can overfit with
small elite populations, \textsf{emphasis} uses a diagonal approximation:
each parameter dimension is updated independently, and the resulting
perturbation is a product of univariate Gaussians.
...
$k \leftarrow 0$;\quad \textit{plat} $\leftarrow 0$;\quad
$f^* \leftarrow -\infty$\;
\BlankLine
\While{$k < T$}{
  ...
  \BlankLine
  \Comment{M-step: resample around elite particles}
  Elite $\leftarrow$ top $\lfloor\rho M\rfloor$ valid particles by $\fhat$\;
  For each non-elite slot $j$: sample $\pi(j) \sim \mathrm{Uniform}(\text{Elite})$
  and set $\pars_j^{(k+1)}$ via \eqref{eq:perturb}\;
  Elite particles are copied unchanged:
  $\pars_j^{(k+1)} \leftarrow \pars_j^{(k)}$ for $j\in\text{Elite}$\;
  \BlankLine
  \Comment{Cool the temperature}
...
\paragraph{Parallelism in the E-step.}
